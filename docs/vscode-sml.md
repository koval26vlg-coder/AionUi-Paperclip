...
VS Code в этой системе не является отдельным агентом. Это рабочая IDE-оболочка для общей папки `D:\AionUi-Paperclip`, через которую можно запускать терминалы Codex/Claude/Antigravity, проверять SML, читать context-pack и работать с файлами общей памяти.
