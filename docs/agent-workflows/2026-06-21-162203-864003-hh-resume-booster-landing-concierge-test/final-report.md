...
- Дата старта и пороги теста хранятся локально: `aion.hhResumeBooster.experiment.v1`.
- Есть explicit decision gate: 14 дней, 30+ лидов, 10+ strong paid intent, 2+ канала, 5+ ролей.
- Есть export JSON.
...
- CLI после gate-фикса не считает `decision_ready=true`, пока `days_complete=false`.
